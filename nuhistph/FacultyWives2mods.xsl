            ...
            <xsl:for-each select="FileSet">
                <mods:mods xmlns:mods="http://www.loc.gov/mods/v3"
                    ...
                    <xsl:for-each select="Description/Metadata[@name='nuhistph.RelationIsPartOf']">
                        <xsl:choose>
                            <xsl:when test="(contains(., 'Faculty') or contains(., 'NU'))">
                    ...
                    <xsl:if test="Description/Metadata[@name='nuhistph.Description'] !=''">
                        
                            <mods:note type="description">
                                <xsl:value-of select="Description/Metadata[@name='nuhistph.Description']"/>
                            </mods:note>
                        
                    </xsl:if>


                    <!--                    <xsl:if test="Description/Metadata[@name='nuhistph.RightsRightsHolder'] !=''">
                        <xsl:for-each
                            select="Description/Metadata[@name='nuhistph.RightsRightsHolder']">
                            <mods:note type="rights">
                                <xsl:value-of select="."/>
                            </mods:note>
                        </xsl:for-each>
                    </xsl:if>-->
                    ...
                    <mods:relatedItem type="original">
                        ...
                        <xsl:for-each
                            ...
                            <xsl:choose>
                                <xsl:when test="contains(., 'White Negative')">
                                    <mods:genre authority="aat">
                                        <xsl:text>black-and-white negatives</xsl:text>
                                    </mods:genre>
                                </xsl:when>
                                <xsl:when test="contains(., 'Color Slide')">
                                    <mods:genre authority="aat">
                                        <xsl:text>color slides</xsl:text>
                                    </mods:genre>
                                </xsl:when>
                                ...
                                <xsl:when test="contains(ancestor::FileSet/FileName, 'pdf')">
                                    <mods:genre authority="aat">
                                        <xsl:text>texts (document genres)</xsl:text>
                                    </mods:genre>
                                </xsl:when>
                        ...
                        <!--                        <xsl:if test="Description/Metadata[@name='nuhistph.Identifier']">
                            <mods:identifier type="F_number">
                                <xsl:value-of
                                    select="Description/Metadata[@name='nuhistph.Identifier']"/>
                            </mods:identifier>
                        </xsl:if>

                        <xsl:if test="Description/Metadata[@name='nuhistph.Url']">
                            <mods:identifier type="A_number">
                                <xsl:value-of select="Description/Metadata[@name='nuhistph.Url']"/>
                            </mods:identifier>
                        </xsl:if>-->
